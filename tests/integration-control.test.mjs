...
test("RU_LIFE exposes neutral control liveness without exposing its dedicated secret", async () => {
  const route = await source("../app/api/integration/control/route.ts");
  assert.match(route, /export async function GET/);
  assert.match(route, /runtime: "RU_LIFE"/);
  assert.match(route, /ru-life-control-v1/);
  ...
  assert.doesNotMatch(route, /MEDICINE_SERVICE_SECRET|health-request|health-response|control-health/);
});

test("authenticated control probe proves RU_LIFE and Application Management share the RU-only secret", async () => {
  const route = await source("../app/api/integration/control/route.ts");
